...
class CQLTrainer(Trainer):
    ...
    def train_iteration(self, num_steps, iter_num=0, print_logs=False, reporter = None):
        ...
        info = self.model.fit(self.mdp_dataset, n_steps=int(num_steps * 3), n_steps_per_epoch=int(int(num_steps * 3) / 10), save_metrics=False,verbose=False, show_progress=False)
